...
extension UsageStoreCoverageTests {
    ...
    func `status indicators and failure gate`() {
        ...
        #expect(!second)
        let third = gate.shouldSurfaceError(onFailureWithPriorData: true)
        #expect(third)
        gate.recordSuccess()
        let withoutPriorData = gate.shouldSurfaceError(onFailureWithPriorData: false)
        #expect(withoutPriorData)
        gate.reset()
